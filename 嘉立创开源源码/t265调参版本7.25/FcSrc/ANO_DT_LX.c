...
u8 User_send_buf[50];//用户数据发送
u8 User_send_buf2[50];//
_dt_st dt;
...
void ANO_DT_Init(void)
{
	...
	dt.fun[0xe2].time_cnt_ms = 0; //设置初始相位，单位1ms--------------------------有两个没改，不知道干嘛的
	
	dt.fun[0xf1].D_Addr = 0xff;
	dt.fun[0xf1].fre_ms = 50;	  //0 由外部触发
	dt.fun[0xf1].time_cnt_ms = 0; //设置初始相位，单位1ms
  
  dt.fun[0xf2].D_Addr = 0xff;
	dt.fun[0xf2].fre_ms = 100;	  //0 由外部触发
...
static void ANO_DT_LX_Data_Receive_Anl(u8 *data, u8 len)
{
	...
	else if (*(data + 2) == 0X03)
	{
		...
		User.Flight.YAW = *((data + 8))|*((data + 9)) << 8;//航向信息YAW
		User.Flight.YAW /= 100;
	...
	{
		int32_t temp=*((int32_t *)(data + 8));//激光高度信息
		if(temp<200){
			User.Flight.alt_cm=temp;
		}
...
//	u32 temp_data_u32;
	...
	case 0xf1: //用户自定义帧
	{
    temp_data = T265.pose_x_cm; //2字节拆两个
		send_buffer[(*_cnt)++] = BYTE0(temp_data);
		send_buffer[(*_cnt)++] = BYTE1(temp_data);
		temp_data = T265.pose_y_cm;
		send_buffer[(*_cnt)++] = BYTE0(temp_data);
		send_buffer[(*_cnt)++] = BYTE1(temp_data);
//		temp_data = User.Flight.KFP_x_cm;
//		send_buffer[(*_cnt)++] = BYTE0(temp_data);
		temp_data = User.Flight.Camera_Flag;//摄像头准备好
		send_buffer[(*_cnt)++] = BYTE0(temp_data);
		...
		temp_data_32 = User.Flight.alt_cm; 
		send_buffer[(*_cnt)++] = BYTE0(temp_data_32);
		send_buffer[(*_cnt)++] = BYTE1(temp_data_32);
		send_buffer[(*_cnt)++] = BYTE2(temp_data_32);
		send_buffer[(*_cnt)++] = BYTE3(temp_data_32);
		temp_data = User.Flight.Success_Find;
		send_buffer[(*_cnt)++] = BYTE0(temp_data);
		temp_data =	User.Flight.YAW;//飞机航向，位置闭环需要考虑
		send_buffer[(*_cnt)++] = BYTE0(temp_data);
		send_buffer[(*_cnt)++] = BYTE1(temp_data);
    temp_data = rt_tar.st_data.vel_x; //期望速度x
		send_buffer[(*_cnt)++] = BYTE0(temp_data);
		send_buffer[(*_cnt)++] = BYTE1(temp_data);
    temp_data = rt_tar.st_data.vel_y;//期望速度y
		send_buffer[(*_cnt)++] = BYTE0(temp_data);
		send_buffer[(*_cnt)++] = BYTE1(temp_data);
  	...
  	case 0xf2: //用户自定义帧
	{
	  ...
	  temp_data = User.Flight.mission_step_test;//飞行步骤
		send_buffer[(*_cnt)++] = BYTE0(temp_data);
		send_buffer[(*_cnt)++] = BYTE1(temp_data);
		temp_data = User.Flight.T265_Flag;//T265准备好
		send_buffer[(*_cnt)++] = BYTE0(temp_data);
		temp_data = User.Flight.place;//当前对点
		send_buffer[(*_cnt)++] = BYTE0(temp_data);
		temp_data = User.Flight.Fly_x;
		send_buffer[(*_cnt)++] = BYTE0(temp_data);
		send_buffer[(*_cnt)++] = BYTE1(temp_data);
		temp_data = User.Flight.Fly_y;
		send_buffer[(*_cnt)++] = BYTE0(temp_data);
		send_buffer[(*_cnt)++] = BYTE1(temp_data);
		temp_data = User.Flight.FireCenter[0];
		send_buffer[(*_cnt)++] = BYTE0(temp_data);
		send_buffer[(*_cnt)++] = BYTE1(temp_data);
		temp_data = User.Flight.FireCenter[1];
		send_buffer[(*_cnt)++] = BYTE0(temp_data);
		send_buffer[(*_cnt)++] = BYTE1(temp_data);
