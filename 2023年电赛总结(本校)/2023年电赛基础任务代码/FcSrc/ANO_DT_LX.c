...
int alt_fu_xm=0,alt_add_xm=0,station_len1=0;


s32 alt_x_positon_xm=0,alt_y_positon_xm=0;
s16 three_rol_xm=0,three_pit_xm=0,three_yaw_xm=0;

//char xm_station_str
//const char *str_xm=xm_station_str;

//signed off_data_x=0,off_data_y=0;
/*==========================================================================
 * 描述    ：凌霄飞控通信主程序
 * 更新时间：2020-01-22 
 * 作者		 ：匿名科创-茶不思
...
void ANO_DT_Init(void)
{
	...
	dt.fun[0xa0].time_cnt_ms = 0; //设置初始相位，单位1ms
	
	dt.fun[0xa1].D_Addr = 0xff;
	dt.fun[0xa1].fre_ms = 0;	  //0 由外部触发
	dt.fun[0xa1].time_cnt_ms = 0; //设置初始相位，单位1ms
...
void ANO_DT_LX_Data_Receive_Prepare(u8 data)
{
	...
	else if (rxstate == 1 && (data == HW_TYPE || data == HW_ALL))//f
	{
		rxstate = 2;
		DT_RxBuffer[1] = data;
	}
	//接收帧CMD字节
	else if (rxstate == 2)//id
	{
		rxstate = 3;
		DT_RxBuffer[2] = data;
	}
...
static void ANO_DT_LX_Data_Receive_Anl(u8 *data, u8 len)
{
	...
	else if (*(data + 2) == 0x05)
	{
		...
		alt_fu_xm = *((int *)(data + 4));
		alt_add_xm = *((int *)(data + 8));
...
//		pwm_to_esc.pwm_m8 = *((u16 *)(data + 18));
	...
	else if (*(data + 2) == 0x08)
	{
		...
		alt_x_positon_xm = *((s32 *)(data + 4));
		alt_y_positon_xm = *((s32 *)(data + 8));
	}
	...
	else if (*(data + 2) == 0x03)
	{
		...
		three_rol_xm = *((s16 *)(data + 4));
		three_pit_xm = *((s16 *)(data + 6));
		three_yaw_xm = *((s16 *)(data + 8));
		three_yaw_xm=three_yaw_xm/100;
...
////////////////////	
//void station_context(u8 buf_xm[],int speak_len )
//{
//	
//		
...
static void Add_Send_Data(u8 frame_num, u8 *_cnt, u8 send_buffer[])
{
	...
	case 0xa0: //CMD命令帧
	{
		send_buffer[(*_cnt)++] = 0x02;//0 is black ;1 is red; 2 is green
//		for (u8 i = 0; i < 10; i++)
//		{
//			send_buffer[(*_cnt)++] = dt.cmd_send.CMD[i];
//		}
		station_len1=strlen(station_buf1);
		
		for (u8 i = 0; i < station_len1; i++)
		{
			send_buffer[(*_cnt)++] =station_buf1[i] ;
		}
...
static void Frame_Send(u8 frame_num, _dt_frame_st *dt_frame)
{
	...
	send_buffer[_cnt++] = dt_frame->D_Addr;//0xff
	send_buffer[_cnt++] = frame_num;//41
	send_buffer[_cnt++] = 0;
	//==
	//add_send_data
...
void LxF1Send(u8 u8val, s16 s16val_1, s16 s16val_2, s16 s16val_3,s16 s16val_4,s16 s16val_5,s16 s16val_6,s16 s16val_7)
{
	...
	f1Buf[_cnt++] = BYTE0(s16val_1);
	f1Buf[_cnt++] = BYTE1(s16val_1);
	f1Buf[_cnt++] = BYTE0(s16val_2);
	f1Buf[_cnt++] = BYTE1(s16val_2);

	f1Buf[_cnt++] = BYTE0(s16val_3);
	f1Buf[_cnt++] = BYTE1(s16val_3);
	f1Buf[_cnt++] = BYTE0(s16val_4);
	f1Buf[_cnt++] = BYTE1(s16val_4);

	f1Buf[_cnt++] = BYTE0(s16val_5);
	f1Buf[_cnt++] = BYTE1(s16val_5);
	f1Buf[_cnt++] = BYTE0(s16val_6);
	f1Buf[_cnt++] = BYTE1(s16val_6);


	f1Buf[_cnt++] = BYTE0(s16val_7);
	f1Buf[_cnt++] = BYTE1(s16val_7);
	//f1Buf[_cnt++] = BYTE0(s32val);
	//f1Buf[_cnt++] = BYTE1(s32val);
	//f1Buf[_cnt++] = BYTE2(s32val);
	//f1Buf[_cnt++] = BYTE3(s32val);
	f1Len = _cnt;
}
static void f1SendCheck(void)
{
	u8 _cnt = 0;
	if(f1Len)
	{
		...
		for(u8 i=0; i<f1Len; i++)
			send_buffer[_cnt++] = f1Buf[i];
		//==
		send_buffer[3] = _cnt - 4;
		//==
		u8 check_sum1 = 0, check_sum2 = 0;
...
void LxStringSend(u8 string_color,char *str)
{
	...
	if(string_color > LOG_COLOR_GREEN)
		string_color = LOG_COLOR_BLACK;
