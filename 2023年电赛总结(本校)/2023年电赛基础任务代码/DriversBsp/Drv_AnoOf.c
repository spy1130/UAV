...
void AnoOF_GetOneByte(uint8_t data)
{
	...
	if (rxstate == 0 && data == 0xAA)//head
	{
		rxstate = 1;
		_datatemp[0] = data;
	}
	else if (rxstate == 1 && (data == HW_TYPE || data == HW_ALL))//add
	{
		rxstate = 2;
		_datatemp[1] = data;
	}
	...
	else if (rxstate == 3 && data < 250)//length
	{
		rxstate = 4;
		_datatemp[3] = data;
		_data_len = data;
	...
	else if (rxstate == 4 && _data_len > 0)//data 5~11
	{
		_data_len--;
	...
	else if (rxstate == 5)//sc
	{
		rxstate = 6;
		_datatemp[4 + _data_cnt++] = data;
	}
	else if (rxstate == 6)//ac
	{
		rxstate = 0;
		_datatemp[4 + _data_cnt] = data;
		//		DT_data_cnt = _data_cnt+5;
		//
		AnoOF_DataAnl(_datatemp, _data_cnt + 5); //分析数据
	}
	else
	{
		rxstate = 0;
